                    ...
                    <div class="row">
                        ...
                        @endif
                            ...
                            <span class="post-meta meta-data {{ 0 == $ad->approved ? 'post-meta-red' : '' }}">
                                 ...
                                 <span class="fa fa-check-circle-o"><a href="{{ url('ads', $ad->id) }}" style="text-decoration: none"> {{$ad->type->name}}</a></span>
                                <br />
                                <span><i class="fa fa-folder-open"></i>
                                    <a
                                            href="/category/{{$ad->category->alias}}">{{$ad->category->title}}</a>

                                </span>
                                <br />
                                <span><i class="fa fa-calendar"></i> {{$ad->created_at->format('d/m/Y')}}</span><br />
                                @if($ad->price)
                               ...
                               @endif
                                ...
                                {!! Form::close() !!}
                                <a href="{{ action('AdsController@edit', $ad->id) }}" class="btn btn-primary btn-sm" style="margin-right: 5px; float: right; background-color: #428bca; ">Редактировать <i class="fa fa-refresh"></i></a>
                                @if($ad->approved == 1)
                                    {!! Form::open(['url'=>["ads/$ad->id/unpublish"],'role'=>'form', 'method' => 'POST', 'style' => 'float:right; margin-top:10px; margin-bottom:5px']) !!}
                                    <button type="submit" class="btn btn-warning btn-sm" > Снять с публикации <i class="fa fa-warning"></i> </button>
                                    {!! Form::close() !!}
                                @endif
                                <hr>
                            </div>
