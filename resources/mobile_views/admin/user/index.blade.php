                    ...
                    <table class="table table-hover" ng-controller="UsersController">
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td>{{$user->balance}}</td>
                                <td>
                                    <a href="{{route('admin.user.edit',['id'=>$user->id])}}" style=" float: right; margin-bottom: 5px" data-toggle="tooltip" data-original-title="Редактировать"
                                       class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                    <div class="btn-group" style="float: right;" role="group" aria-label="...">
                                        {!! Form::open(['route'=>['admin.user.destroy',$user->id], 'class'=>'form-horizontal confirm',
                                        'role'=>'form', 'method' => 'DELETE']) !!}
                                        <button type="submit" class="btn btn-danger confirm-btn"  style="margin-bottom: 5px" data-toggle="tooltip" data-original-title="Удалить"><i class="fa fa-trash-o"></i></button>
                                        {!! Form::close() !!}
                                        {!! Form::open(['route'=>['admin.user.block',$user->id], 'class'=>'form-horizontal confirm',
                                        'role'=>'form', 'method' => 'POST']) !!}
                                        @if (0 == $user->blocked)
                                            {!! Form::hidden('blocked', 1) !!}
                                            <button type="submit" style="margin-top: 3px; margin-bottom: 5px" class="btn btn-danger confirm-block" data-toggle="tooltip" data-original-title="Заблокировать"><i class="fa fa-ban"></i></button>
                                        @else
                                            {!! Form::hidden('blocked', 0) !!}
                                            <button type="submit" style="margin-top: 3px; margin-bottom: 5px" class="btn btn-success confirm-unblock" data-toggle="tooltip" data-original-title="Разблокировать"><i class="fa fa-ban"></i></button>
                                        @endif
                                        {!! Form::close() !!}


                                    </div>
                                    <a href="{{route('admin.user.addshop',['id'=>$user->id])}}"  style=" float: right;margin-bottom: 5px" data-toggle="tooltip" data-original-title="Добавить магазин"
